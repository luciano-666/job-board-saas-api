...
from src.modules.shared.presentation.dependencies import get_authentication_repository
...
async def decode_nested_access_token(token: str) -> Session:
    try:
        ...
        session: Session = await access_token_entity_mapper(
...
async def decode_nested_refresh_token(token: str) -> Session:
    try:
        ...
        session: Session = await refresh_token_entity_mapper(
            json.loads(inner.claims)
        )  # FIX
...
async def _build_access_token_lookup(session: Session) -> SessionLookup:
...
async def _build_refresh_token_lookup(session: Session) -> SessionLookup:
...
async def authenticate_user(
    ...
    try:
        ...
        db_session: Optional[Session] = await repository.get_access_token_by_session(
            await _build_access_token_lookup(session)
        )
...
async def authenticate_refresh(
    ...
    try:
        ...
        db_session: Optional[Session] = await repository.get_refresh_token_by_session(
            await _build_refresh_token_lookup(session)
        )
